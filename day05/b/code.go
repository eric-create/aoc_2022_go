...
func operate(stacks *[][]rune, operation Operation) {
	...
	crates := sourceStack[len(sourceStack)-operation.Number:]

	(*stacks)[operation.Target] = append(targetStack, crates...)
	(*stacks)[operation.Source] = sourceStack[:len(sourceStack)-operation.Number]
}
