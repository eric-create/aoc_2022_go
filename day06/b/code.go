...
func main() {
	...
	for _, line := range lines {
		fmt.Println(GetMarkerEnd(line, 4))
		fmt.Println(GetMarkerEnd(line, 14))
		fmt.Println("---")
	}
}

func GetMarkerEnd(line string, markerLen int) int {
	series := []rune(line)
	markerEnd := 0

	for i := 0; ; i++ {
		pool := series[i : i+markerLen]
		duplicatePoint := GetDuplicatePoint(pool)

		if duplicatePoint == 0 {
			markerEnd = i + (markerLen - 1)
			break
		}

		// We want to preceed from the position behind the first element of the duplicate.
		i += duplicatePoint - 1
		if i+markerLen > len(series) {
			return 0
		}
	}
