...
func main() {
	...
	// PrintTree(node, "")
	fmt.Println("---")
	toBeFreed := node.Size - 40000000
	candidates := GetCandidates(node, toBeFreed)
	sort.Ints(candidates)
	fmt.Println(candidates[0])

}

func GetCandidates(node Node, toBeFreed int) []int {
	candidates := []int{}

	for _, node := range node.Children {
		candidates = append(candidates, GetCandidates(*node, toBeFreed)...)
	}

	if node.Size >= toBeFreed {
		return append(candidates, node.Size)
	} else {
		return candidates
	}
}
