	...
	for i := 1; i <= rounds; i++ {
		for _, monkey := range monkeys {
			for len(monkey.Items) > 0 {
				...
				peer := monkey.GetPeer(test)
				monkey.Throw(item, peer, monkeys)
			}
		}
	}
...
func (monkey *Monkey) GetPeer(test bool) int {
	if test {
		return monkey.PeerTrue
	} else {
		return monkey.PeerFalse
	}
}

func (monkey *Monkey) Throw(item int, peer int, monkeys []*Monkey) {
	monkeys[peer].Items = append(monkeys[peer].Items, item)
}
